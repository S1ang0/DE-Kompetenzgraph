...
// bridges in between — this lays out the FIN×FMB story spatially.
const CLUSTER_ORDER = [
...
export default function ForceGraph({
  dataset, filters, selectedId, onSelect, activeProfile,
  showLabels, showHulls,
}) {
  ...
  const colorOf = useMemo(() => {
    const m = {};
    dataset.clusters.forEach((c) => (m[c.key] = c.color));
    return m;
  }, [dataset]);
  const nameOf = useMemo(() => {
    const m = {};
    dataset.clusters.forEach((c) => (m[c.key] = c.name));
    return m;
  }, [dataset]);
  ...
  // ---- build simulation + static DOM (on data / size change) ----
  useEffect(() => {
    const { w, h } = dims;
    ...
    const clusterKeys = CLUSTER_ORDER.filter((k) => nodes.some((n) => n.cluster === k));

    function drawHulls() {
      const data = clusterKeys.map((k) => {
        const pts = nodes.filter((n) => n.cluster === k).map((n) => [n.x, n.y]);
        return { key: k, pts };
      }).filter((d) => d.pts.length);
      // expand hull outward from centroid
      const padded = (pts, pad) => {
        if (pts.length < 3) {
          const [cx, cy] = pts.length === 1 ? pts[0]
            : [(pts[0][0] + pts[1][0]) / 2, (pts[0][1] + pts[1][1]) / 2];
          return d3.range(10).map((i) => {
            const a = (i / 10) * 2 * Math.PI;
            return [cx + Math.cos(a) * (pad + 16), cy + Math.sin(a) * (pad + 16)];
          });
        }
      ...
      const hsel = gHull.selectAll("path").data(data, (d) => d.key).join("path")
        .attr("class", "hull")
        .attr("fill", (d) => colorOf[d.key]).attr("fill-opacity", 0.07)
        .attr("stroke", (d) => colorOf[d.key]).attr("stroke-opacity", 0.22).attr("stroke-width", 1);
      hsel.attr("d", (d) => hullLine(padded(d.pts, 22)));

      const lsel = gHullLabel.selectAll("g").data(data, (d) => d.key).join((enter) => {
        const g = enter.append("g");
        g.append("text").attr("class", "hull-label").attr("text-anchor", "middle");
        return g;
      });
      lsel.each(function (d) {
        const cx = d3.mean(d.pts, (p) => p[0]);
        const top = d3.min(d.pts, (p) => p[1]);
        d3.select(this).select("text")
          .attr("x", cx).attr("y", top - 14)
          .attr("fill", d3.color(colorOf[d.key]).darker(1.2))
          .text(nameOf[d.key]);
      });
    }
  ...
  }, [dataset, dims.w, dims.h]);

  // keep latest showHulls available to the tick closure
  ...
  }, [showLabels, dataset, dims]);

  // ---- restyle on selection / profile / filters (no sim restart) ----
  useEffect(() => {
    const s = selRef.current;
    if (!s?.node) return;
    ...
    const emphasized = (d) => {
      ...
      return d.id === selectedId || neigh?.has(d.id);
    };
    ...
    node
      ...
      .attr("stroke-width", (d) => {
        if (d.id === selectedId) return 2.6;
        if (profSets?.core.has(d.id)) return 2;
        return 1.1;
      })
    ...
    link.attr("opacity", (d) => {
      ...
      if (selectedId) {
        const touches = d.source.id === selectedId || d.target.id === selectedId;
        return touches ? 0.5 : 0.04;
      }
      return d.cross_faculty ? 0.16 : 0.10;
    }).attr("stroke", (d) =>
      selectedId && (d.source.id === selectedId || d.target.id === selectedId) ? "#2B3A55" : "#1C1B19"
    );

    label.attr("opacity", (d) => {
      if (!showLabels) return 0;
      if (!visible(d)) return 0;
      if (selectedId) return emphasized(d) ? 1 : 0;
      return 1;
    });
  }, [selectedId, profSets, filters, adj, colorOf, showLabels, dataset, dims.w, dims.h]);
  ...
  return (
    <div className="canvas" ref={wrapRef}>
      ...
      <div className="canvas__tools">
        <button className="zoombtn" onClick={() => zoomBy(1.3)} aria-label="Hineinzoomen" title="Hineinzoomen">
          <Plus />
        </button>
        <button className="zoombtn" onClick={() => zoomBy(1 / 1.3)} aria-label="Herauszoomen" title="Herauszoomen">
          <Minus />
        </button>
        <button className="zoombtn" onClick={resetZoom} aria-label="Ansicht zurücksetzen" title="Ansicht zurücksetzen">
          <Target />
        </button>
      </div>
      {tip && (
        <div className="tooltip" style={{ left: tip.x, top: tip.y }} role="presentation">
          ...
          <div className="tooltip__meta">
            {tip.d.faculty} · {tip.d.cp} CP · {tip.d.language.toUpperCase()} · {nameOf[tip.d.cluster]}
          </div>
        </div>
      )}
    </div>
